...
There is also another way to format text in GDScript, namely the `String.format() <class_String_method_format>` method. It replaces all occurrences of a key in the string with the corresponding value. The method can handle arrays or dictionaries for the key/value pairs.
...
> [!NOTE]
> In Godot's C++ code, GDScript format strings can be accessed using the `vformat()` helper function in the `Variant<class_Variant>` header.
