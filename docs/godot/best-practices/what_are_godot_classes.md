...
The engine provides built-in classes like `Node <class_Node>`. You can extend those to create derived types using a script.
...
<class_ClassDB>`. This database provides runtime access to class information. `ClassDB` contains information about classes like:
...
Attaching a `Script <class_Script>` to your object extends the methods, properties, and signals available from the `ClassDB`.

> [!NOTE]
> Even scripts that don't use the `extends` keyword implicitly inherit from the engine's base `RefCounted <class_RefCounted>` class. As a result, you can instantiate scripts without the `extends` keyword from code. Since they extend `RefCounted` though, you cannot attach them to a `Node <class_Node>`.
