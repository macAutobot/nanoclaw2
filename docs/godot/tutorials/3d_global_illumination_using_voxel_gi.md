...
Not sure if VoxelGI is suited to your needs? See `doc_introduction_to_global_illumination_comparison` for a comparison of GI techniques available in Godot 4.
...
<figure>
<img src="img/gi_none.webp" alt="VoxelGI disabled." />
<figcaption aria-hidden="true">VoxelGI disabled.</figcaption>
</figure>

<figure>
<img src="img/gi_voxel_gi.webp" alt="VoxelGI enabled." />
<figcaption aria-hidden="true">VoxelGI enabled.</figcaption>
</figure>
...
> [!NOTE]
> For meshes with the **Static** bake mode, the VoxelGI baking system is not able to make use of custom shaders (`class_ShaderMaterial`). These meshes will be considered to be pure black, only acting as light blockers. You can make VoxelGI take custom shaders into account by using the **Dynamic** bake mode for these objects, but this has a performance cost.
>
> For `class_BaseMaterial3D`, some properties are currently ignored during baking. This can impact visuals if the material's albedo or emission texture was designed around using certain UV mappings:
>
> - **UV1 \> Offset**
> - **UV1 \> Scale**
> - **UV1 \> Triplanar**
...
See `doc_introduction_to_global_illumination_gi_mode_recommendations` for general usage recommendations.
