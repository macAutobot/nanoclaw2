...
SDFGI supports dynamic lights, but *not* dynamic occluders or dynamic emissive surfaces. Therefore, SDFGI provides better real-time ability than `baked lightmaps <doc_using_lightmap_gi>`, but worse real-time ability than `VoxelGI <doc_using_voxel_gi>`.
...
Not sure if SDFGI is suited to your needs? See `doc_introduction_to_global_illumination_comparison` for a comparison of GI techniques available in Godot 4.
...
<figure>
<img src="img/gi_none.webp" alt="SDFGI disabled." />
<figcaption aria-hidden="true">SDFGI disabled.</figcaption>
</figure>

<figure>
<img src="img/gi_sdfgi.webp" alt="SDFGI enabled." />
<figcaption aria-hidden="true">SDFGI enabled.</figcaption>
</figure>
...
- **Dynamic (not supported with SDFGI):** The mesh won't be taken into account in SDFGI generation. The mesh will receive indirect lighting from the scene, but it will not contribute indirect lighting to the scene. *This acts identical to theDisabled*\* bake mode when using SDFGI.\*
