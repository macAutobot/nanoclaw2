...
- ![release](img/template_release.svg) Available in editor builds, debug export templates and release export templates.
- ![debug](img/template_debug.svg) Available in editor builds and debug export templates only.
- ![extended](img/template_extended.svg) Only available in editor builds, and export templates compiled with `disable_path_overrides=false`.
...
| `--no-header` | ![release](img/template_release.svg) Do not print engine version and rendering method header on startup. |
...
| `--recovery-mode` | ![editor](img/editor.svg) "Start the editor in recovery mode, which disables features that can typically cause startup crashes, such as tool scripts, editor plugins, GDExtension addons, and others. |
| `--debug-server <uri>` | ![editor](img/editor.svg) Start the editor debug server (`<protocol>://<host/IP>[:<port>]`, e.g. `tcp://127.0.0.1:6007`) |
| `--dap-port <port>` | ![editor](img/editor.svg) Use the specified port for the GDScript Debug Adapter Protocol. Recommended port range `[1024, 49151]`. |
| `--lsp-port <port>` | ![editor](img/editor.svg) Use the specified port for the GDScript Language Server Protocol. Recommended port range `[1024, 49151]`. |
...
| `-l`, `--language <locale>` | ![release](img/template_release.svg) Use a specific locale. `<locale>` follows the format `language_Script_COUNTRY_VARIANT` where language is a 2 or 3-letter language code in lowercase and the rest is optional. See `doc_locales` for more details. |
| `--path <directory>` | ![extended](img/template_extended.svg) Path to a project (`<directory>` must contain a "project.godot" file). |
| `--scene <path>` | ![extended](img/template_extended.svg) Path or UID of a scene in the project that should be started. |
| `--main-pack <file>` | ![extended](img/template_extended.svg) Path to a pack (.pck) file to load. |
| `--render-thread <mode>` | ![release](img/template_release.svg) Render thread mode ("unsafe", "safe", "separate"). See `Thread Model <class_ProjectSettings_property_rendering/driver/threads/thread_model>` for more details. |
...
| `--audio-output-latency <ms>` | ![release](img/template_release.svg) Override audio output latency in milliseconds (default is 15 ms). Lower values make sound playback more reactive but increase CPU usage, and may result in audio cracking if the CPU can't keep up. |
...
| `--log-file` | ![release](img/template_release.svg) Write output/error log to the specified path instead of the default location defined by the project. `<file>` path should be absolute or relative to the project directory. |
...
| `--wid <window_id>` | ![release](img/template_release.svg) Request parented to window. |
| `--accessibility <mode>` | ![release](img/template_release.svg) Select accessibility mode \['auto" (when screen reader is running, default), "always", "disabled'\]. |
...
| `--ignore-error-breaks` | ![release](img/template_release.svg) If debugger is connected, prevents sending error breakpoints. |
...
| `--gpu-validation` | ![release](img/template_release.svg) Enable graphics API `validation layers <doc_vulkan_validation_layers>` for debugging. |
...
| `--generate-spirv-debug-info` | ![debug](img/template_debug.svg) Generate SPIR-V debug information. This allows source-level shader debugging with RenderDoc. |
| `--extra-gpu-memory-tracking` | ![debug](img/template_debug.svg) Enables additional memory tracking (see class reference for <span class="title-ref">RenderingDevice.get_driver_and_device_memory_report()</span> and linked methods). Currently only implemented for Vulkan. Enabling this feature may cause crashes on some systems due to buggy drivers or bugs in the Vulkan Loader. See <https://github.com/godotengine/godot/issues/95967> |
| `--accurate-breadcrumbs` | ![debug](img/template_debug.svg) Force barriers between breadcrumbs. Useful for narrowing down a command causing GPU resets. Currently only implemented for Vulkan. |
...
| `--debug-canvas-item-redraw` | ![debug](img/template_debug.svg) Display a rectangle each time a canvas item requests a redraw (useful to troubleshoot low processor mode). |
| `--max-fps <fps>` | ![release](img/template_release.svg) Set a maximum number of frames per second rendered (can be used to limit power usage). A value of 0 results in unlimited framerate. |
| `--frame-delay <ms>` | ![release](img/template_release.svg) Simulate high CPU load (delay each frame by \<ms\> milliseconds). Do not use as a FPS limiter; use `--max-fps` instead. |
...
| `--editor-pseudolocalization` | ![editor](img/editor.svg) Enable pseudolocalization for the editor and the project manager. |
...
|----|----|
| Command | Description |
| `-s`, `--script <script>` | ![extended](img/template_extended.svg) Run a script. `<script>` must be a resource path relative to the project (`myscript.gd` will be interpreted as `res://my_script.gd`) or an absolute filesystem path (for example, on Windows: `C:/tmp/my_script.gd`). |
| `--main-loop <main_loop_name>` | ![extended](img/template_extended.svg) Run a MainLoop specified by its global class name. |
| `--check-only` | ![extended](img/template_extended.svg) Only parse for errors and quit (use with `--script`). |
...
| `--export-release <preset> <path>` | ![editor](img/editor.svg) Export the project in release mode using the given preset and output path. The preset name should match one defined in "export_presets.cfg". `<path>` should be absolute or relative to the project directory, and include the filename for the binary (e.g. "builds/game.exe"). The target directory must exist. |
...
| `--export-patch <preset> <path>` | ![editor](img/editor.svg) Export pack with changed files only. See `--export-pack` description for other considerations. |
| `--patches <paths>` | ![editor](img/editor.svg) List of patches to use with `--export-patch`. The list is comma-separated. |
| `--install-android-build-template` | ![editor](img/editor.svg) Install the Android build template. Used in conjunction with `--export-release` or `--export-debug`. |
...
| `--gdextension-docs` | ![editor](img/editor.svg) Rather than dumping the engine API, generate API reference from all the GDExtensions loaded in the current project (used with `--doctool`). |
...
| `--dump-gdextension-interface` | ![editor](img/editor.svg) Generate GDExtension header file "gdextension_interface.h" in the current folder. This file is the base file required to implement a GDExtension. |
| `--dump-gdextension-interface-json` | ![editor](img/editor.svg) Generate a JSON dump of the GDExtension interface named "gdextension_interface.json" in the current folder. |
...
| `--dump-extension-api-with-docs` | ![editor](img/editor.svg) Generate JSON dump of the Godot API like the previous option, but including documentation. |
...
| `--test [--help]` | ![editor](img/editor.svg) Run `unit tests <doc_unit_testing>` (requires compiling the engine with `tests=yes`). Use `--test --help` for more information. |
...
\# Editor with C# support (will be available as <span class="title-ref">godot-mono</span> in <span class="title-ref">PATH</span>): scoop install godot-mono
...
\# Editor with C# support (will be available as <span class="title-ref">godot-mono</span> in <span class="title-ref">PATH</span>): brew install godot-mono
