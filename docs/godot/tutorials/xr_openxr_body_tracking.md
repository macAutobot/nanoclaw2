...
These trackers are identified by their roles which are assigned to them when configured. Simply add `XRController3D <class_xrcontroller3d>` nodes as children to the `XROrigin3D <class_xrorigin3d>` node and assign one of the following trackers:
