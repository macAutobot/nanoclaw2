...
To pause the game the pause state must be set. This is done by assigning `true` to the `SceneTree.paused <class_SceneTree_property_paused>` property:
...
Each node in Godot has a "Process Mode" that defines when it processes. It can be found and changed under a node's `Node <class_Node>` properties in the inspector.
...
public override void <span id="ready">Ready</span>() { ProcessMode = Node.ProcessModeEnum.Pausable; }
...
func <span id="on_pause_button_pressed">on_pause_button_pressed</span>():  
...
func <span id="on_close_button_pressed">on_close_button_pressed</span>():  
