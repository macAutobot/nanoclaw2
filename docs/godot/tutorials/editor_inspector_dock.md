...
- **Expand Non-Default**: Only expands the sections where the original value is different than the current value (the properties with a revert icon (![undo](img/inspector_dock_revert.webp))).
...
> [!TIP]
> If a node has exported variables in its attached script, you will also see these in the inspector. The first image in this section has one for the Player node: <span class="title-ref">Action Suffix</span>. See `doc_gdscript_exports` for more on this topic.

<div class="seealso">

Refer to `doc_customizing_editor` for dock customization options.
