<div class="meta" keywords="Signal">
...
Consider a player character that can rotate and shoot towards the mouse. Every time the mouse button is clicked, we create an instance of the bullet at the player's location. See `doc_instancing` for details.
...
public partial class Bullet : Area2D { public Vector2 Velocity { get; set; } = Vector2.Right;

> public override void <span id="physicsprocess">PhysicsProcess</span>(double delta) { Position += Velocity \* (float)delta; }
...
var bullet_instance = Bullet.instantiate() get_parent().add_child(bullet_instance)
...
Node bulletInstance = Bullet.Instantiate(); GetParent().AddChild(bulletInstance);
...
public partial class Player : Sprite2D { \[Signal\] public delegate void ShootEventHandler(PackedScene bullet, float direction, Vector2 location);

> private PackedScene <span id="bullet">bullet</span> = GD.Load\<PackedScene\>("res://Bullet.tscn");
>
> public override void <span id="input">Input</span>(InputEvent @event) { if (@event is InputEventMouseButton mouseButton) { if (mouseButton.ButtonIndex == MouseButton.Left && mouseButton.Pressed) { EmitSignal(SignalName.Shoot, <span id="bullet">bullet</span>, Rotation, Position); } } }
>
> public override void <span id="process">Process</span>(double delta) { LookAt(GetGlobalMousePosition()); }
...
func <span id="on_player_shoot">on_player_shoot</span>(Bullet, direction, location):  
var spawned_bullet = Bullet.instantiate() add_child(spawned_bullet) spawned_bullet.rotation = direction spawned_bullet.position = location spawned_bullet.velocity = spawned_bullet.velocity.rotated(direction)
...
private void OnPlayerShoot(PackedScene bullet, float direction, Vector2 location) { var spawnedBullet = bullet.Instantiate\<Bullet\>(); AddChild(spawnedBullet); spawnedBullet.Rotation = direction; spawnedBullet.Position = location; spawnedBullet.Velocity = spawnedBullet.Velocity.Rotated(direction); }
