...
In your scene tree select your left hand `XRController3D <class_xrcontroller3d>` node. Now click on the **instantiate Child Scene** button to add a child scene. Click the **addons** toggle so the addons folder can be searched. Then search for `left_hand_low.tscn`, and select it.
