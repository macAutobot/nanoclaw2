...
<img src="img/engine_compilation_configuration_editor_detect.webp" alt="Opening the Engine Compilation Configuration Editor" />
<figcaption aria-hidden="true">Opening the Engine Compilation Configuration Editor</figcaption>
</figure>
...
<figure class="align-center">
<img src="img/engine_compilation_configuration_editor_detected.webp" alt="Updated features list after using feature detection (example from the 3D platformer demo)" />
<figcaption aria-hidden="true">Updated features list after using feature detection (example from the 3D platformer demo)</figcaption>
</figure>
...
This file can be passed as a SCons option when `compiling <doc_compiling_index>` export templates:
...
- Features that are used in `GDExtensions <doc_gdextension>`, unless the language binding allows for defining used classes and the extension makes use of the functionality. See [GH-104129](https://github.com/godotengine/godot/pull/104129) for details.
...
You can achieve further size reductions by passing other options that reduce binary size. See `doc_optimizing_for_size` for more information.
