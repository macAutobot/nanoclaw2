...
> [!NOTE]
> Godot supports high dynamic range *output*. You can read more about this on the `doc_hdr_output` page.
>
...
After all the rendering is done, the scene linear render requires transforming to a suitable output such as an sRGB display. To do this, enable sRGB conversion in the current `Environment <class_Environment>` (more on that below).
...
HDR settings can be found in the `Environment <class_Environment>` resource. Most of the time, these are found inside a `WorldEnvironment <class_WorldEnvironment>` node or set in a Camera node. For more information, see `doc_environment_and_post_processing`.
