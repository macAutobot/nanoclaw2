...
If you want to run a dedicated server for your project on a machine that doesn't have a GPU or display server available, you'll need to run Godot with the `headless` display server and `Dummy` `audio driver <class_ProjectSettings_property_audio/driver/driver>`.
...
<img src="img/exporting_for_dedicated_servers_export_mode.webp" alt="Choosing the **Export as dedicated server** export mode in the export preset" />
<figcaption>Choosing the <strong>Export as dedicated server</strong> export mode in the export preset</figcaption>
</figure>
...
<figure class="align-center">
<img src="img/exporting_for_dedicated_servers_export_resources.webp" alt="Choosing resources to keep, keep with stripped visuals or remove" />
<figcaption aria-hidden="true">Choosing resources to keep, keep with stripped visuals or remove</figcaption>
</figure>
...
If you `exported the project <doc_exporting_for_dedicated_servers_exporting_project>` using the **Export as dedicated server** export mode (or have added `dedicated_server` as a custom feature tag), you can use the `dedicated_server` feature tag to detect whether a dedicated server PCK is being used:

<div class="tabs">

.. code-tab:: gdscript

\# Note: Feature tags are case-sensitive. if OS.has_feature("dedicated_server"): \# Run your server startup code here... pass
...
// Note: Feature tags are case-sensitive. if (OS.HasFeature("dedicated_server")) { // Run your server startup code here... }
...
\# Run your server startup code here... \# \# Using this check, you can start a dedicated server by running \# a Godot binary (editor or export template) with the <span class="title-ref">--headless</span> \# command-line argument. pass
...
if (DisplayServer.GetName() == "headless") { // Run your server startup code here... // // Using this check, you can start a dedicated server by running // a Godot binary (editor or export template) with the <span class="title-ref">--headless</span> // command-line argument. }
...
\# Run your server startup code here... \# \# Using this check, you can start a dedicated server by running \# a Godot binary (editor or export template) with the <span class="title-ref">--server</span> \# command-line argument. pass
...
if (OS.GetCmdlineUserArgs().Contains("--server")) { // Run your server startup code here... // // Using this check, you can start a dedicated server by running // a Godot binary (editor or export template) with the <span class="title-ref">--server</span> // command-line argument. }
