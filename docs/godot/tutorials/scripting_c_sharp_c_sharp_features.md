...
- One of `GODOT_LINUXBSD`, `GODOT_WINDOWS`, `GODOT_OSX`, `GODOT_ANDROID`, `GODOT_IOS`, `GODOT_WEB` depending on the OS. These names may change in the future. These are created from the `get_name()` method of the `OS <class_OS>` singleton, but not every possible OS the method returns is an OS that Godot with .NET runs on.
