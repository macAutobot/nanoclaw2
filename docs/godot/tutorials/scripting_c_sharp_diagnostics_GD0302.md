...
A generic type is specified for a generic type argument when a `Variant-compatible type <c_sharp_variant_compatible_types>` is expected, but the specified generic type is not annotated with the `[MustBeVariant]` attribute.
