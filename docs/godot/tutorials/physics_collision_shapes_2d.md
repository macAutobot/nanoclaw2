...
You can define the shape of a `class_PhysicsBody2D` by adding one or more `CollisionShape2Ds <class_CollisionShape2D>` or `CollisionPolygon2Ds <class_CollisionPolygon2D>` as *direct* child nodes. Indirect child nodes (i.e. children of child nodes) will be ignored and won't be used as collision shapes. Also, note that you must add a `class_Shape2D` *resource* to collision shape nodes in the Inspector dock.
