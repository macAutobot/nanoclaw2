...
In Godot, class members can be exported. This means their value gets saved along with the resource (such as the `scene <class_PackedScene>`) they're attached to. They will also be available for editing in the property editor. Exporting is done by using the `[Export]` attribute.
...
> [!NOTE]
> Exporting properties can also be done in GDScript, for information on that see `doc_gdscript_exports`.
...
Display a visual representation of the `ease<class_@GlobalScope_method_ease>` function when editing.
...
Custom node classes can also be exported directly. The filtering behavior depends on whether the custom class is a `global class <doc_c_sharp_global_classes>`.
...
The drop-down menu will be limited to AnimationNode and all its derived classes. Custom resource classes can also be used, see `doc_c_sharp_global_classes`.

It must be noted that even if the script is not being run while in the editor, the exported properties are still editable. This can be used in conjunction with a `script in "tool" mode <doc_gdscript_tool_mode>`.
...
If you want to create a clickable button in the inspector, you can use the `[ExportToolButton]` attribute. This exports a Callable property or field as a clickable button. Since this runs in the editor, usage of the `[Tool]
<doc_running_code_in_the_editor>` attribute is required. When the button is pressed, the callable is called:
...
You can also set an icon for the button with a second argument. If specified, an icon will be fetched via `GetThemeIcon() <class_Control_method_get_theme_icon>`, from the `"EditorIcons"` theme type.
...
As explained in the `C# Variant <doc_c_sharp_variant>` documentation, only certain C# arrays and the collection types defined in the `Godot.Collections` namespace are Variant-compatible, therefore, only those types can be exported.
...
When changing an exported variable's value from a script in `doc_gdscript_tool_mode`, the value in the inspector won't be updated automatically. To update it, call `NotifyPropertyListChanged() <class_Object_method_notify_property_list_changed>` after setting the exported variable's value.
...
Before reading further, you should get familiar with the way properties are handled and how they can be customized with `_Set() <class_Object_private_method__set>`, `_Get() <class_Object_private_method__get>`, and `_GetPropertyList() <class_Object_private_method__get_property_list>` methods as described in `doc_accessing_data_or_logic_from_object`.
