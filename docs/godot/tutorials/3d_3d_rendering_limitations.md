...
- If using the Forward+ or Forward Mobile rendering methods, enable `Use Debanding<class_ProjectSettings_property_rendering/anti_aliasing/quality/use_debanding>` in **Project Settings \> Rendering \> Anti Aliasing**. This applies a fullscreen debanding shader as a post-processing effect and is very cheap.
...
<figcaption aria-hidden="true">Color banding comparison (contrast increased for more visibility)</figcaption>
</figure>
...
<figcaption aria-hidden="true">Z-fighting comparison (before and after tweaking the scene by offsetting the Label3D away from the floor)</figcaption>
</figure>
...
<figcaption aria-hidden="true">Transparency sorting comparison (alpha-blended materials on the left, alpha scissor materials on the right)</figcaption>
</figure>
