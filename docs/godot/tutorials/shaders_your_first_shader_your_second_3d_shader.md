...
A difference between the vertex function and a fragment function is that the vertex function runs per vertex and sets properties such as `VERTEX` (position) and `NORMAL`, while the fragment shader runs per pixel and, most importantly, sets the `ALBEDO` color of the `MeshInstance3D<class_MeshInstance3D>`.
...
<doc_shading_language>` doc and the `Spatial Shaders <doc_spatial_shader>` doc. Also look at more advanced tutorials in the `Shading section
<toc-learn-features-shading>` and the `3D <toc-learn-features-3d>` sections.
