...
Check the `doc_instancing` tutorial to learn about Godot's approach to scene instancing.
...
<class_Node_method_get_node>` method. For this to work, the child node must be present in the scene tree. Getting it in the parent node's `_ready()` function guarantees that.
...
private Sprite2D <span id="sprite2d">sprite2D</span>; private Camera2D <span id="camera2d">camera2D</span>;

public override void <span id="ready">Ready</span>() { base.\_Ready();

> <span id="sprite2d">sprite2D</span> = GetNode\<Sprite2D\>("Sprite2D"); <span id="camera2d">camera2D</span> = GetNode\<Camera2D\>("Camera2D");
...
private AnimationPlayer <span id="animationplayer">animationPlayer</span>;

public override void <span id="ready">Ready</span>() { base.\_Ready();

> <span id="animationplayer">animationPlayer</span> = GetNode\<AnimationPlayer\>("ShieldBar/AnimationPlayer");
...
private Sprite2D <span id="sprite2d">sprite2D</span>;

public override void <span id="ready">Ready</span>() { base.\_Ready();

> <span id="sprite2d">sprite2D</span> = new Sprite2D(); // Create a new Sprite2D. AddChild(<span id="sprite2d">sprite2D</span>); // Add it as a child of this node.
...
<span id="sprite2d.queuefree">sprite2D.QueueFree</span>();
...
2.  Creating an instance of the loaded `PackedScene <class_PackedScene>` resource.
...
At that point, `scene` is a packed scene resource, not a node. To create the actual node, you need to call `PackedScene.instantiate()
<class_PackedScene_method_instantiate>`. It returns a tree of nodes that you can use as a child of your current node.
