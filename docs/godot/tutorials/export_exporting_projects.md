...
Finally, Godot has a simple but efficient system for `creating DLCs as extra package files <doc_exporting_pcks>`.
...
See `doc_command_line_tutorial` for more information about using Godot from the command line.
...
- Readable and writable using tools normally present on the user's operating system. This can be useful to make modding easier (see also `doc_exporting_pcks`).

> [!WARNING]
> Due to a [known bug](https://github.com/godotengine/godot/pull/42123), when using a ZIP file as a pack file, the exported binary will not try to use it automatically. Therefore, you have to create a *launcher script* that the player can double-click or run from a terminal to launch the project:
>
> ``` none
> :: launch.bat (Windows)
> @echo off
