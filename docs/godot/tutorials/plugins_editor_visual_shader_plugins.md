...
Visual Shader plugins are used to create custom `class_VisualShader` nodes in GDScript.
...
Create a Sprite2D and assign a `class_ShaderMaterial` to its material slot:
...
Create a script which derives from `class_VisualShaderNodeCustom`. This is all you need to initialize your plugin.
