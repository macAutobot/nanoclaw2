...
- **Custom Shaped Elements**: Custom shapes can be created with `Polygon2D <class_Polygon2D>` allowing UV animation, deformations, etc.
...
This small cross in the middle of the `Sprite2D <class_Sprite2D>` is the rotation pivot:
...
The `RemoteTransform2D <class_RemoteTransform2D>` node transforms nodes somewhere else in the hierarchy. This node applies its own transform (including any transformation it inherits from its parents) to the remote node it targets.
...
The first step is creating an endpoint node. Any kind of node will do, but `Marker2D <class_Marker2D>` is preferred because it's visible in the editor. The endpoint node will ensure that the last bone has orientation.
...
The following section will be a collection of tips for creating animation for your cutout rigs. For more information on how the animation system in Godot works, see `doc_introduction_animation`.
